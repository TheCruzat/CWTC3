...
function drawCard($i) {
	...
	<a href="' . get_permalink($i->ID) . '" class="relative rounded-[0.75rem] overflow-hidden md:flex w-full md:items-end text-center h-[360px] md:h-[420px] max-md:block max-md:mb-[calc(var(--rails)*1.5)]" style="background:url(' . esc_url(bg_img($i->ID, 'large')) . ') 50% 50% / cover no-repeat;" class="md:h-[60vh]">
		<img src="' . esc_url(bg_img($i->ID, 'thumbnail')) . '" alt="' . esc_attr($i->post_title, 'cwtc3') . '" class="md:hidden h-full object-cover w-full" />
